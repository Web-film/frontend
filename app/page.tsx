...
export default async function Home() {
  ...
  const filmMove = await getFilm(24, 1, "movie");
  const filmTv = await getFilm(24, 1, "tv");
    ...
    <main>
      <div className="container flex flex-col gap-[50px]">
        ...
        <div className="rounded-2xl bg-[image:var(--bg-list)] flex flex-col p-[2rem] gap-[2rem]">
          <ListFilm classNameSlide={"slide-populate"} films={popularFilms} title="Top 10 bộ phim thịnh hành" />
          <ListFilm classNameSlide={"slide-new"} films={newFilms} title="Phim mới cập nhật" />
          <ListFilm classNameSlide={"slide-new"} films={filmMove} title="Phim lẻ mới nhất" />
          <ListFilm classNameSlide={"slide-new"} films={filmTv} title="Phim bộ mới nhất" />
        </div>
      </div>
    </main>
  );
