...
interface Props {
  params: { type: string; genre: string; page: string };
}

async function ListTypeGenre({ params }: Props) {
  const { type, genre, page } = await params;
  const currentPage = parseInt(page || "1");
  const limit = 35;

  const getDatas = await getFilm({ limit, page: currentPage, genreId: type });
  ...
  return (
    <div className="container">
      ...
      <div className="py-6 flex gap-4 items-center w-full justify-center">
        {currentPage > 1 && (
          <Link href={`/list/${type}/${genre}/${currentPage - 1}`}>
